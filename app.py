...
def initialize_vector_db():
    ...
    dataset_path = 'cybersecurity_facts.txt'
    
    # Check if file exists to prevent crashing
    if not os.path.exists(dataset_path):
        # Creating a fallback file for demonstration if it doesn't exist
        with open(dataset_path, 'w') as f:
            f.write("SQL Injection (SQLi) Description: Injects malicious SQL into application queries to read, modify, or delete database data.\n")
            f.write("Cross-Site Scripting (XSS) Description: Injects JavaScript into trusted web pages to steal cookies or sessions.\n")
            f.write("Remote Code Execution (RCE) Description: Allows attackers to execute arbitrary code on a target system.\n")
...
# User Input
input_query = st.text_input("Ask me a question about cybersecurity:", placeholder="e.g., What is SQL injection?")
